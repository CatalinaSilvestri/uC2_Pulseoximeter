...
static PulseState_t PulseAmpel_GetState(uint16_t bpmValue)
{
    if (bpmValue >= 60 && bpmValue <= 90)
    {
        return PULSE_STATE_GREEN;
    }

    if ((bpmValue >= 50 && bpmValue < 60) || (bpmValue > 90 && bpmValue <= 120))
    {
...
void PulseAmpel_Update(uint16_t bpmValue)
{
    PulseState_t state = PulseAmpel_GetState(bpmValue);
...
static void Display_Update(const char *line1, const char *line2)
{
    char l1[17];
    char l2[17];
...
/*
 * Wait for RIGHT button before starting measurement.
 */
static void WaitForStartButton(void)
{
    uint16_t mirrorCounter = 0;

    Display_Update("RIGHT = Start", "Press button");
    ...
    while (1)
    {
        ...
        {
            Display_Update("Sensor start...", "Please wait");
            _delay_ms(500);
            return;
        }
        ...
        /*
         * Important:
         * Repeat LCD mirror while waiting.
         * Otherwise Python misses the first packet if it connects after AVR reset.
         */
        mirrorCounter++;
...
int main(void)
{
    ...
    /*
     * Wait here until RIGHT button is pressed.
     */
    WaitForStartButton();
    ...
    Display_Update("Place finger", "BPM: --");
        ...
        if ((timeMs - lastLcdMs) >= LCD_UPDATE_MS)
    {
        ...
        if (!fingerPresent)
        {
            snprintf(line1, sizeof(line1), "No finger");
        }
        else if (saturated)
        {
            snprintf(line1, sizeof(line1), "Too bright");
        }
        ...
        {
            snprintf(line1, sizeof(line1), "Finger: beat *");
        }
        else
        {
            snprintf(line1, sizeof(line1), "Finger detected");
        }

        if (saturated)
        {
            snprintf(line2, sizeof(line2), "Lower LED curr");
        }
        else if (!fingerPresent)
        {
            snprintf(line2, sizeof(line2), "BPM: --");
        }
        else if (!measurementReady)
        {
            uint16_t remaining;
            ...
            snprintf(line2, sizeof(line2), "Wait:%2us", remaining);
        }
        else if (bpm == 0)
        {
            snprintf(line2, sizeof(line2), "BPM: --");
        ...
        else
        {
            ...
            {
                snprintf(line2, sizeof(line2), "B:%3u O2:%3u%%", bpm, spo2);
            }
            else
            {
                snprintf(line2, sizeof(line2), "B:%3u O2:--%%", bpm);
            }
        }

        Display_Update(line1, line2);
    }
