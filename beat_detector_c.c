...
int32_t FilterBuLp1_step(FilterBuLp1* f, int32_t x)
{
    ...
    f->v[1] = q15_mul_i32(FILTER_B0_Q15, x)
            + q15_mul_i32(FILTER_A1_Q15, f->v[0]);
...
int32_t DCRemover_step(DCRemover* d, uint16_t x)
{
    ...
    /*
     * Avoid the huge startup spike.
     *
     * Original formula:
     *
     *     dcw = x + alpha * dcw
     *
     * For constant x, steady-state is:
     *
     *     dcw = x / (1 - alpha)
     *
     * For alpha = 0.95:
     ...
     *     dcw = x * 20
     *
     * This makes the first returned AC value 0 instead of a false pulse.
     */
    if (!d->initialized)
    {
        if (d->alpha_q15 == DC_REMOVER_ALPHA_Q15)
        {
            d->dcw = raw * 20L;
        }
        else
        {
            /*
             * Generic fallback for other alpha values:
             *
             * steady = x / (1 - alpha)
             *
             * In Q15:
             * denominator = 32768 - alpha_q15
             */
            int32_t denom = 32768L - d->alpha_q15;
            ...
            {
                d->dcw = (int32_t)(((int64_t)raw * 32768L) / denom);
            }
        }
     ...
     *     return dcw - olddcw;
     */
    d->dcw = raw + q15_mul_i32(d->alpha_q15, d->dcw);
